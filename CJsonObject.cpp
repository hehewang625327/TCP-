...
bool CJsonObject::AddItemToObject(const char *key, const char *value)
{
	printf("1:%s:%s\n",key,value);
	cJSON *item=cJSON_CreateString(value);
	printf("2:%s\n",item->valuestring);
	cJSON_AddItemToObject(m_pJsonData,key, item);
	printf("3:%s\n",m_pJsonData->valuestring);
	return true;
}
...
cJSON *basic_cjson_pkg(void)
{
    ...
    cJSON_AddStringToObject(root, "action", "ddff"); 
